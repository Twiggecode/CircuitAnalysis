...
To run the program, you must use a command line argument. The program accepts two formats for the command line argument. The standard format is: python main.py test.net test.csv
where main.py is this file, test.net is the input file and test.csv is the output file. The extended command line format is:   python main.py -i test -f
This format will also accept an input file of test.net and and output file of test.csv. The '-f' flag represents that the output data will be calculated and displayed in
...
The units of the output quantities can be specified in the <OUTPUT> block. Unit prefixes can be used. dBV, dBW etc can be used to display a quantity in decibel/phase format.
...
if sys.argv[1]=='-i': # extended command line format detected
    
    
    
    f = open(sys.argv[2]+'.net',"r")
    f1 =f.readlines()
    f.close()
    ...
    if commentflag!=1: 
       ...
       try:
           ...
           p=int(tempslice)
               ...
               f = open(sys.argv[2],"w+") # write blank file
